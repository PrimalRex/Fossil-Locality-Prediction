...
# This script helps us do most of our evaluation metrics without repeating them multiple times, also ensures consistent format
# in all metrics whether its model evaluation or trying to understand a random guessing approach
...
# Prints the metrics and returns the values
def getMetrics(yTest, testPredictions, testBinaryPredictions, prefix="Test"):
    print(f"----------------------------")
    print(f"{prefix} Overall Accuracy: {accuracy_score(yTest, testBinaryPredictions):.4f}")
    print(f"{prefix} Precision: {precision_score(yTest, testBinaryPredictions, zero_division=0):.4f}")
    print(f"{prefix} Recall: {recall_score(yTest, testBinaryPredictions, zero_division=0):.4f}")
    ...
    print(f"{prefix} AUC-ROC: {roc_auc_score(yTest, testPredictions):.4f}")
    print(f"----------------------------")
    return (precision_score(yTest, testBinaryPredictions, zero_division=0),
            recall_score(yTest, testBinaryPredictions, zero_division=0),
            f1_score(yTest, testBinaryPredictions, zero_division=0),
            fossiliferousConfidenceScore(yTest, testPredictions),
            roc_auc_score(yTest, testPredictions))
