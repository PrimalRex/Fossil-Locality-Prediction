...
def processClimateResources(CLIMATE_RESOURCE_DIR, alpha, shift, climateType):
    ...
    for file, filename in tqdm(enumerate(os.listdir(CLIMATE_RESOURCE_DIR)), total=fileCount, desc=f"Building Composite {climateType}"):
...
precipitationIntensities, start, end = processClimateResources(CLIMATE_PRECIPITATION_RESOURCE_DIR, alpha, shift, "Precipitation")
...
temperatureIntensities, start, end = processClimateResources(CLIMATE_TEMPERATURE_RESOURCE_DIR, alpha, shift, "Temperature")
